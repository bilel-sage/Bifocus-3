...
import { Play, Pause, X, Plus, Clock, Zap, Battery, Moon, Sun, BarChart3, Target, Eye, Calendar, Edit2, Trash2, Circle, CheckCircle2, PlayCircle } from 'lucide-react';
...
const BiFocus = () => {
  ...
  const loadData = async () => {
    try {
      const tasksResult = await window.storage.get(`tasks-${activeSpace}`).catch(() => null);
      const statsResult = await window.storage.get(`stats-${activeSpace}`).catch(() => null);
  ...
  const saveData = async (newTasks, newStats) => {
    try {
      await window.storage.set(`tasks-${activeSpace}`, JSON.stringify(newTasks));
      await window.storage.set(`stats-${activeSpace}`, JSON.stringify(newStats));
    } catch (error) {
      console.error('Error saving:', error);
    }
  };

  // ============ TIMER LOGIC - SIMPLE ET STABLE ============
  useEffect(() => {
    ...
    // Démarrer seulement si actif et pas en pause
  ...
  return (
    <div className={`app ${darkMode ? 'dark' : 'light'}`}>
                        ...
                        <div key={task.id} className={`task-card-mini ${task.status} ${task.priority}`}>
                          ...
                          <div className="task-content-mini">
                            ...
                            <div className="task-meta-mini">
                              <span><Clock size={12} /> {task.estimatedMinutes || 30}min</span>
      ...
      {/* Floating Timer Widget - DÉPLAÇABLE PARTOUT */}
      {timerActive && (
        <div 
